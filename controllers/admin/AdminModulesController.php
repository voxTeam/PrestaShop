...
{
	const CACHE_FILE_STRUCTURE = '/config/json/structure.json';
	const CACHE_FILE_CATALOG = '/config/json/catalog.json';
	...
	{
		if ($this->isFresh(self::CACHE_FILE_CATALOG, 86400) && !Tools::getValue('force_reload_cache'))
			die(file_get_contents(_PS_ROOT_DIR_.self::CACHE_FILE_CATALOG));

		if (self::$is_prestashop_up)
		{
			$data = Tools::jsonEncode(array());
			self::$is_prestashop_up = false;
			file_put_contents(_PS_ROOT_DIR_.self::CACHE_FILE_CATALOG, $data);
			die($data);
		}
		else
	...
	public function ajaxProcessGetStructure()
	{
		if ($this->isFresh(self::CACHE_FILE_STRUCTURE, 86400) && !Tools::getValue('force_reload_cache'))
			die(file_get_contents(_PS_ROOT_DIR_.self::CACHE_FILE_STRUCTURE));
		...
		if (self::$is_prestashop_up)
		{
			...
			self::$is_prestashop_up = false;
			file_put_contents(_PS_ROOT_DIR_.self::CACHE_FILE_STRUCTURE, $data);
			die($data);
		}
		else
			die(Tools::jsonEncode('Error'));
